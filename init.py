...
class Scrapy():
    ...
    def EnableCookie(self, enableProxy):
        ...
        if enableProxy:
            ...
            opener = urllib2.build_opener(
                proxy_support, cookie_support, urllib2.HTTPHandler)
            print 'proxy enabled'
        else:
    ...
    def get_rand(self, url):
        ...
        passwordName = HTML.fromstring(login_page).xpath(
    ...
    def login(self, cookie_filename=None):
        ...
        if len(HTML.fromstring(page).xpath('//img')) <= 2:
            ...
            # 提取capID
            capId = HTML.fromstring(page).xpath(
                '//input[@name="capId"]/@value')[0]
            ...
            # 再次发送请求
            req = urllib2.Request(
                loginUrl, urllib.urlencode(thePostDict), self.headers)
            res = urllib2.urlopen(req)
            page = res.read()
...
if aaa.login() == True:
    ...
    print '登陆成功'
    getTodayAll.getTodayCommand(cookie)